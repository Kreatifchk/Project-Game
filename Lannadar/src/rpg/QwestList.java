...
public class QwestList {
	...
	//ѕеред добавлением измени размер массива в классе Game
	
	public QwestList() {
		//јргументы: id, name, textN, textK, request, status, exp, lastId(опционально)
		//ƒл¤ убийства: nameMonster, number и общие в том же пор¤дке
		//ƒл¤ разговора idNPC и далее общие
		...
		Game.qwest[3] = new QwestKilling("«омби", 3, 3, name(3), textN(3), textK(3), request(3), 1, 20, 2);
		Game.qwest[4] = new QwestTalk(3, 4, name(4), textN(4), textK(4), request(4), 1, 14, 3);
		Game.qwest[5] = new QwestKilling("«омби", 2, 5, name(5), textN(5), textK(5), request(5), 1, 25, 4);
		Game.qwest[6] = new QwestKilling("—келет", 3, 6, name(6), textN(6), textK(6), request(6), 1, 35, 5);
		Game.qwest[7] = new QwestTalk(5, 7, name(7), textN(7), textK(7), request(7), 1, 40, 6);
		Game.qwest[8] = new QwestKilling("Ќекромант", 5, 8, name(8), textN(8), textK(8), request(8), 1, 60, 7);
